...
uint16_t GPIO_ReadFromInputPort(GPIO_RegDef_t *pGPIOx)  // 16 pin -> uint16
{
    uint16_t value;
 ...
 */
void GPIO_IRQInterruptConfig(uint8_t IRQNumber, uint8_t EnorDi) // line interrup in vector table
{
    if(EnorDi == ENABLE)
    {
        if(IRQNumber <= 31)
...
void GPIO_IRQPriorityConfig(uint8_t IRQNumber, uint32_t IRQPriority) // position in vector table
{
    uint8_t iprx = IRQNumber / 4;
	uint8_t iprx_section  = IRQNumber %4 ;

    *(  NVIC_PR_BASE_ADDR + iprx ) |=  ( IRQPriority << (8*iprx_section + 4) );  // only 4 high bit 
}
void GPIO_IRQHandling(uint8_t PinNumber)
{
    //clear the exti pending bit - pending register to next interrupt
